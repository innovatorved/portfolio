...
const utapi = UPLOADTHING_TOKEN
  ? new UTApi({ token: UPLOADTHING_TOKEN })
  : null;
...
async function uploadAsset(assetPath: string): Promise<string | null> {
  if (!utapi) {
    console.warn(
      "⚠️ UPLOADTHING_TOKEN not set, skipping upload for:",
      assetPath,
    );
    return null;
  }
  ...
  for (const file of files) {
    ...
    blocks = blocks.map((block: any) => {
      if (
        ...
        const url =
          textObj.href || (content.startsWith("http") ? content : null);
    ...
    const properties: any = {
      ...
      PublishedAt: {
        date: {
          start: metadata.publishedAt || new Date().toISOString().split("T")[0],
        },
      },
...
async function main() {
  ...
  if (!UPLOADTHING_TOKEN) {
    console.warn(
      "⚠️  UPLOADTHING_TOKEN is missing. Files will NOT be uploaded.",
    );
  }
...
async function updateDatabaseSchema(dbId: string, name: string) {
  ...
  if (!SECRET) {
    console.warn(
      `⚠️ WEBHOOK_SECRET not set. Skipping Deploy formula update for ${name}.`,
    );
    return;
  }

  const workerUrl =
    "https://portfolio-cms-worker-beta.innovatorved.workers.dev";
